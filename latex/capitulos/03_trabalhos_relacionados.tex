...
Em consonância com a orientação metodológica adotada nesta pesquisa, empregou-se uma visão sistematizada da literatura, com explicitação do protocolo de busca e seleção dos estudos. Essa opção metodológica é particularmente relevante porque reduz a subjetividade inerente às revisões puramente narrativas e permite justificar, de modo mais transparente, a constituição do conjunto de referências analisadas. Além disso, tal procedimento favorece o alinhamento entre o problema de pesquisa, os critérios de escolha dos estudos e o quadro comparativo apresentado ao final da seção.

\subsection{Protocolo de busca e seleção dos estudos}

A construção desta seção foi orientada por um protocolo de revisão da literatura com foco aplicado, voltado à identificação de estudos próximos ao problema central do presente trabalho: a utilização de representações neurais para reconstrução e visualização imersiva de cenas 3D, considerando simultaneamente fidelidade visual, custo computacional e viabilidade de integração em realidade virtual.
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.70\textwidth]{figuras/jin2025_fig1.png}}
