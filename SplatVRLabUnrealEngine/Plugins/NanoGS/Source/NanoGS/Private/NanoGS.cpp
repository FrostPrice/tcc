	...
	//------------------------------------------------------------------
		...
		GraphBuilder.AddPass(
			 ...
			 IntermediateTexture, VelocityTexture, DepthTexture](FRHICommandListImmediate& RHICmdList)
			{
				...
				auto BeginGaussianRenderPass = [&RHICmdList, IntermediateTexture, VelocityTexture, DepthTexture]()
				{
					FRHIRenderPassInfo RenderPassInfo;
					RenderPassInfo.ColorRenderTargets[0].RenderTarget = IntermediateTexture->GetRHI();
					RenderPassInfo.ColorRenderTargets[0].Action = ERenderTargetActions::Clear_Store;

					if (VelocityTexture)
					{
						RenderPassInfo.ColorRenderTargets[1].RenderTarget = VelocityTexture->GetRHI();
						RenderPassInfo.ColorRenderTargets[1].Action = ERenderTargetActions::Load_Store;
					}

					if (DepthTexture)
					{
						RenderPassInfo.DepthStencilRenderTarget.DepthStencilTarget = DepthTexture->GetRHI();
						RenderPassInfo.DepthStencilRenderTarget.Action = EDepthStencilTargetActions::LoadDepthStencil_StoreDepthStencil;
						RenderPassInfo.DepthStencilRenderTarget.ExclusiveDepthStencil = FExclusiveDepthStencil::DepthWrite_StencilWrite;
					}

					RHICmdList.BeginRenderPass(RenderPassInfo, TEXT("GaussianSplat_RenderToIntermediate"));
				};
